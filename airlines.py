...
st.sidebar.title('US airline sentiment')
...
if not st.sidebar.checkbox('Hide',True):
    ...
    if select == "Histogram":
        hist = px.bar(sentiment_count,x='Sentiment',y='Tweets',color='Tweets',height=500)
        st.plotly_chart(hist)
    else:
        pie = px.pie(sentiment_count,values='Tweets',names='Sentiment')
        st.plotly_chart(pie)
...
choice = st.sidebar.multiselect('Pick airlines',('US Airways','United','American','Soutwest','Delta','Virgin America'),key='0')
if len(choice) > 0:
